...
def get_change_point_y(im):
    ...
    for y in xrange(int(0.13 * height), height, 20):
...
def recognize(im):
    ...
    if len(change_point_y) < 3:
        bottom = height
    else:
        bottom = change_point_y[-3]
    t_im = im.crop((0.1 * width, 0.18 * height, 0.95 * width, bottom))  # x0, y0, x1, y1
    # t_im.show()
...
if __name__ == '__main__':
    im = Image.open('img/2018-01-09 20:35:48.png')
